...
def analyze_process(process_name: str, connections: list, all_destinations: dict) -> dict:
    ...
    # --- 1. Destinații externe unice (văzute DOAR la acest proces, la niciun altul) ---
    unique_ips = {c["remote_ip"] for c in external_connections}
    unique_to_this_process = [
        ip for ip in unique_ips
        if len(all_destinations.get(ip, set())) == 1
    ]
    if unique_to_this_process:
        # dacă TOATE conexiunile externe merg către o singură destinație unică, e mai suspect
        ratio = sum(1 for c in external_connections if c["remote_ip"] in unique_to_this_process) / n_external
        if ratio > 0.8:
            report["risk_score"] += NEW_DESTINATION_WEIGHT
            report["reasons"].append(
                f"{ratio*100:.0f}% din traficul extern merge către destinații necunoscute/unice: {unique_to_this_process[:3]}"
            )
