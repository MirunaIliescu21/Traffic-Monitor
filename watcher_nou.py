...
def collect_and_notify(conn: sqlite3.Connection, process_cache: dict, notified_pairs: set,
    ...
    for conn_info in all_conns:
        ...
        if is_inbound:
            ...
            inbound_key = (name, remote_ip, local_port)
...
def run_risk_analysis(conn: sqlite3.Connection) -> None:
    ...
    for process_name, conns in by_process.items():
        ...
        if report["risk_score"] >= RISK_ALERT_THRESHOLD:
            # afișăm TOATE motivele, nu doar primele 2 (care erau mereu
            # liniile generice de deduplicare/filtrare, nu motivul real)
            reasons_text = "\n      ".join(report["reasons"])
            print(f"[ALERTĂ] {process_name}: risc {report['risk_score']}/100")
            print(f"      {reasons_text}")
            # pt. notificarea desktop, folosim un rezumat mai scurt (doar motivele
            # care au contribuit efectiv la scor - conțin cuvintele cheie relevante)
